...
var tagArticleList=new Vue({
	el:'#search-article-list',
	data:{
		pageNumList:[],
		count:100,
		keyWord:'',
	...
	computed:{
		getPage:function(){
			...
			return function (page,pageSize){
				axios({
					method:'get',
					url:'/queryAllBlog?page='+(page-1)*5+'&pageSize='+pageSize+'&keyWord='+self.keyWord
				}).then(function(resp){
				...
				axios({
					method:'get',
					url:'/queryAllBlogCount?keyWord='+self.keyWord,
				}).then(function(resp){
	...
	created:function(){
		var  keyWord=location.search.indexOf('?')>-1 ? location.search.split('?')[1]:'';
