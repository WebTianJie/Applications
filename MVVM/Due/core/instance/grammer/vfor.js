...
function analysisInstructions(vm,instructions,elm,parent){
	...
	for(let i=0;i<dataSet.length;i++){
		let temDom=document.createElement(elm.nodeName);
		temDom.innerHTML=elm.innerHTML;
		let env=analysisKV(insSet[0],dataSet[i],i);
		temDom.setAttribute('env',JSON.stringify(env));
		parent.elm.appendChild(temDom);
		resultSet.push(temDom);
	}
	return resultSet;
}
